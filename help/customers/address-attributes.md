...
1. Para comprobar la precisión de los datos especificados en el campo antes de guardar el registro, establezca **[!UICONTROL Input Validation]** en el tipo de datos que se permitirá en el campo. Los valores disponibles dependen del _[!UICONTROL Input Type]_especificado.
   ...
   >[!NOTE]
   >
   >Para atributos de dirección de cliente definidos por el sistema, como _Teléfono_, _Ciudad_ y _Calle_, Commerce exige la validación integrada del lado del servidor independientemente de la configuración de **[!UICONTROL Input Validation]**. Estas reglas predeterminadas limitan los caracteres permitidos para cada campo (por ejemplo, los números de teléfono solo pueden contener dígitos, espacios y determinados símbolos). La configuración **[!UICONTROL Input Validation]** agrega más restricciones, pero no puede anular la validación integrada, que no se puede deshabilitar a través de la IU de administración.
...
1. Para aplicar un filtro de preprocesamiento a los valores especificados en un campo de texto, área de texto o tipo de entrada de varias líneas, establezca **[!UICONTROL Input/Output Filter]** en uno de los siguientes:
   ...
   - `Strip HTML Tags`: quita las etiquetas HTML del texto. Este filtro puede ayudar a limpiar los datos que se pegan en un campo de otra fuente que incluye etiquetas de HTML.
...
| [!UICONTROL Input Validation] | La selección de opciones viene determinada por el tipo de entrada. Opciones: <br/>**`None`**- El campo no tiene validación de entrada durante la entrada de datos.<br/>**`Alphanumeric`** - Acepta cualquier combinación de números (0-9) y caracteres alfabéticos (a-z, A-Z) durante la entrada de datos. <br/>**`Alphanumeric with Space`**: permite que los espacios en la dirección de la calle cumplan los requisitos de longitud máxima del operador. Durante el cierre de compra, los clientes pueden introducir letras (a-z, A-Z), números (0-9) y espacios. Los espacios adicionales se recortan cuando se guarda la dirección.<br/>**`Numeric Only`** : solo acepta números (0-9) durante la entrada de datos. <br/>**`Alpha Only`**- Acepta solamente caracteres alfabéticos (a-z, A-Z) durante la entrada de datos.<br/>** URL **: solo acepta una URL durante la entrada de datos.<br/>**`Email`** - Acepta solamente una dirección de correo electrónico durante la entrada de datos. <br/>**`Length Only`**: valida la entrada en función de la longitud de los datos introducidos en el campo.<br/><br/>** Nota:**Para atributos definidos por el sistema como _Teléfono_, _Ciudad_ y _Calle_, siempre se aplica la validación integrada del lado del servidor además de cualquier configuración de **[!UICONTROL Input Validation]**. Estas reglas predeterminadas restringen los caracteres permitidos para cada campo y no se pueden anular. La configuración **[!UICONTROL Input Validation]**solo agrega más restricciones. |
| [!UICONTROL Input/Output Filter] | Aplica un filtro de preprocesamiento a los valores introducidos en un campo de texto, área de texto o tipo de entrada de varias líneas antes de guardar el registro. Opciones: <br/>**`None`**: no aplica ningún filtro al texto introducido en el campo.<br/>**`Strip HTML Tags`**: quita las etiquetas HTML del texto. Este filtro puede ayudar a limpiar los datos que se pegan en un campo de otra fuente que incluye etiquetas de HTML. <br/>**`Escape HTML Entities`**: convierte los caracteres especiales encontrados en el texto en una secuencia de escape de HTML válida, como `amp;`. Las secuencias de escape se encierran entre un signo &amp; y un punto y coma, y se utilizan frecuentemente para comillas tipográficas, símbolos de copyright y símbolos de marca comercial. Las secuencias de escape también se utilizan para identificar caracteres como los símbolos menor que (`<`) y mayor que (`>`), y el carácter ampersand que también se utilizan en el código. Este filtro puede ayudar a limpiar los caracteres especiales que a veces se pegan en los campos de base de datos desde procesadores de texto. |
...
| [!UICONTROL Grid Filter Condition Type] | Especifica las condiciones de coincidencia de filtros para los atributos en operaciones de búsqueda desde la cuadrícula. No afecta al campo _[!UICONTROL Search by keyword]_de la cuadrícula. Opciones: `Partial Match` / `Prefix Match` / `Full Match` |
