...
description: Revise los ajustes de configuración de la [!UICONTROL Braintree] en la sección [!UICONTROL Sales] &gt; [!UICONTROL Payment Methods] de la administración de Commerce.
...
>Para las versiones de Adobe Commerce y Magento Open Source anteriores a la 2.4.0, se recomendó que los comerciantes instalen y configuren la extensión oficial de integración de pagos de Braintree desde [Commerce Marketplace](https://marketplace.magento.com/catalogsearch/result/?q=braintree) para reemplazar la integración principal. A partir de la versión 2.4.0, la extensión de se incluye ahora en la versión principal.
><br/><br/>
>Al migrar a Commerce 2.4, los comerciantes deben desinstalar la extensión distribuida en Marketplace (`paypal/module-braintree` o `gene/module-braintree`) y actualice cualquier personalización de código para utilizar el `PayPal_Braintree` namespace en lugar de `Magento_Braintree`. Las opciones de configuración de la extensión agrupada para Commerce y la extensión distribuida en el Commerce Marketplace se mantienen. Los pagos realizados con esas versiones de la extensión se capturan, anulan o devuelven con normalidad.
><br/><br/>
>Si actualiza a Commerce 2.4.0 y no utiliza la extensión de Commerce Marketplace recomendada en la versión 2.3.x anterior, la función de varias direcciones no funciona con la versión 2.4.0 de Braintree. Cuando un comprador selecciona _enviar a varias direcciones_ , la forma de pago del Braintree no aparece. La extensión de Commerce Marketplace recomendada anteriormente para 2.3.x tiene este problema de varias direcciones.
...
| [!UICONTROL Payment Action] | Vista de tienda | Determina la acción realizada por el Braintree cuando se procesa un pago. Opciones: <br/>**`Authorize`**- Se autorizan los fondos en la tarjeta de crédito del cliente, pero no se transfieren desde la cuenta. Se crea un pedido en el administrador de la tienda. Más tarde puede capturar la venta y crear una factura.<br/>**`Intent Sale`** (anteriormente `Authorize and Capture` en versiones anteriores): el Braintree autoriza y captura los fondos de la tarjeta de crédito del cliente, y se crea un pedido y una factura en el administrador de la tienda. |
| [!UICONTROL Sandbox Merchant ID] | Vista de tienda | Este es el identificador único de toda la cuenta de puerta de enlace de la zona protegida. También conocido como _identificador público_ o _ID de producción_ Sin embargo, su ID de comerciante es diferente para las puertas de enlace de producción y de zona protegida. Este campo aparece cuando la variable _[!UICONTROL Environment]_el campo está configurado como `Sandbox`. |
| [!UICONTROL Sandbox Public Key] | Vista de tienda | Identificador público específico del usuario que restringe el acceso a los datos cifrados. Cada usuario asociado con la puerta de enlace del Braintree de la zona protegida tiene su propia clave pública de la zona protegida. Este campo aparece cuando la variable _[!UICONTROL Environment]_el campo está configurado como `Sandbox`. |
| [!UICONTROL Sandbox Private Key] | Vista de tienda | Identificador privado específico del usuario que restringe el acceso a los datos cifrados. Cada usuario asociado con la puerta de enlace del Braintree de la zona protegida tiene su propia clave privada para la zona protegida. Este campo aparece cuando la variable _[!UICONTROL Environment]_el campo está configurado como `Sandbox`. |
| [!UICONTROL Merchant ID] | Vista de tienda | Este es el identificador único de toda la cuenta de puerta de enlace, incluidas las cuentas de varios comerciantes que pueden estar en la puerta de enlace. También conocido como _identificador público_ o _ID de producción_ Sin embargo, su ID de comerciante es diferente para las puertas de enlace de producción y de zona protegida. Este campo aparece cuando la variable _[!UICONTROL Environment]_el campo está configurado como `Production`. |
| [!UICONTROL Public Key] | Vista de tienda | Identificador público específico del usuario que restringe el acceso a los datos cifrados. Cada usuario asociado con la puerta de enlace de Braintree tiene su propia clave pública. Este campo aparece cuando la variable _[!UICONTROL Environment]_el campo está configurado como `Production`. |
| [!UICONTROL Private Key] | Vista de tienda | Identificador privado específico del usuario que restringe el acceso a los datos cifrados. Cada usuario asociado con la puerta de enlace de Braintree tiene su propia clave privada. Este campo aparece cuando la variable _[!UICONTROL Environment]_el campo está configurado como `Production`. |
...
| [!UICONTROL Bypass Fraud Protection Threshold] | Sitio web | `Advanced Fraud Protection` las comprobaciones se omiten cuando se alcanza o supera el valor de umbral. Si deja este campo en blanco, se deshabilita esta opción. |
...
| [!UICONTROL Credit Card Types] | Sitio web | Especifica cada tarjeta de crédito que acepta como pago a través del Braintree. Mantener presionado `Ctrl` (o `Command` en Mac) para seleccionar una combinación de tarjetas. Opciones: `American Express` / `Visa` / `MasterCard` / `Discover` / `JCB` / `Diners` / `Maestro International` |
...
| Campo | [Ámbito](../../getting-started/websites-stores-views.md#scope-settings) | Descripción |
|--- |--- |--- |
| [!UICONTROL Enable Webhook] | Sitio web | Para habilitar la funcionalidad de webhook para la protección contra fraudes, pagos ACH y métodos de pago locales. Opciones: `Yes` / `No` |
| [!UICONTROL Fraud Protection URL] | Sitio web | Añada esta dirección URL a su cuenta de Braintree como [!UICONTROL Webhook Destination URL]. **Esta URL debe ser segura y accesible públicamente.** |
| [!UICONTROL Fraud Protection Approve Order Status] | Sitio web | Cuando el Braintree aprueba la protección contra el fraude, el estado del pedido seleccionado se asigna al pedido comercial. Este estado se utiliza para actualizar el estado del pedido donde se utiliza el método de pago ACH y cuando pasa a `SETTLED` en Braintree. |
| [!UICONTROL Fraud Protection Reject Order Status] | Sitio web | Cuando el Braintree rechaza la protección contra el fraude, el estado del pedido seleccionado se asigna al pedido comercial. Este estado se utiliza para actualizar el estado del pedido en el que se utiliza el método de pago ACH y cuándo `SETTLEMENT` es `DECLINED` en Braintree. |

{style="table-layout:auto"}

## [!UICONTROL Country Specific Settings]

![Configuración específica del país](./assets/payment-methods-braintree-country-specific-config.png)<!-- zoom -->
...
| [!UICONTROL Country Specific Credit Card Types] | Sitio web | Identifica las tarjetas de crédito aceptadas por país para los pagos procesados por el Braintree. Se guarda un registro para cada país. Opciones: <br/>**`Country`**- Elige el país.<br/>**`Allowed Card Types`** - Seleccione cada tarjeta de crédito que se acepte del país como pago a través del Braintree. <br/>**`Add`**- Agregue una línea para permitir tarjetas de crédito para un país diferente.<br/>**`Action`** - Elimina el registro de tarjetas de crédito permitidas para el país. |

{style="table-layout:auto"}

## [!UICONTROL ACH through Braintree]

![BRAINTREE pasante de ACH](./assets/payment-methods-braintree-ach-config.png)<!-- zoom -->

| Campo | [Ámbito](../../getting-started/websites-stores-views.md#scope-settings) | Descripción |
|--- |--- |--- |
| [!UICONTROL Enabled ACH Direct Debit] | Sitio web | Determina si [!DNL ACH Direct Debit] se incluye como forma de pago a través del Braintree. Opciones: `Yes` / `No` |
| [!UICONTROL Sort Order] | Sitio web | Determina el orden en que [!DNL ACH Direct Debit] aparece con otras formas de pago durante el proceso de pago. |

{style="table-layout:auto"}

## [!UICONTROL Apple Pay through Braintree]

![Braintree de Apple Pay through](./assets/payment-methods-braintree-applepay-config.png)<!-- zoom -->

| Campo | [Ámbito](../../getting-started/websites-stores-views.md#scope-settings) | Descripción |
|--- |--- |--- |
| [!UICONTROL Enable ApplePay through Braintree] | Sitio web | Determina si Apple Pay se incluye como forma de pago a través del Braintree. Opciones: `Yes` / `No` <br/><br/> El dominio debe ser [verificado primero en la cuenta de Braintree](https://developer.paypal.com/braintree/docs/guides/apple-pay/configuration/javascript/v3). |
| [!UICONTROL Payment Action] | Sitio web | Determina la acción realizada por el Braintree cuando se procesa un pago. Opciones: <br/>**`Authorize`**- Los fondos en la tarjeta del cliente están autorizados, pero no transferidos desde la cuenta del cliente. Se crea un pedido en el administrador de la tienda. Más tarde puede capturar la venta y crear una factura.<br/>**`Intent Sale`** - Los fondos en la tarjeta del cliente son autorizados y capturados por el Braintree, y un pedido y una factura se crean en el administrador de su tienda. **_Nota:_** Esto era `Authorize and Capture` en 2.3.x y versiones anteriores. |
...
| [!UICONTROL Allowed Payment Method] | Sitio web | Seleccione el método de pago local que desea activar. Opciones: `Bancontact` / `EPS` / `giropay` / `iDeal` / `Klarna Pay Now` / `SOFORT` / `MyBank` / `P24` / `SEPA/ELV Direct Debit` (aún no compatible) |
...
| Campo | [Ámbito](../../getting-started/websites-stores-views.md#scope-settings) | Descripción |
|--- |--- |--- |
| [!UICONTROL Enabled GooglePay through Braintree] | Sitio web | Determina si [!DNL Google Pay] el pago se incluye como forma de pago a través del Braintree. Opciones: `Yes` / `No` |
| [!UICONTROL Payment Action] | Sitio web | Determina la acción realizada por el Braintree cuando se procesa un pago. Opciones: <br/>**`Authorize`**- Los fondos en la tarjeta del cliente están autorizados, pero no transferidos desde la cuenta del cliente. Se crea un pedido en el administrador de la tienda. Más tarde puede capturar la venta y crear una factura.<br/>**`Intent Sale`** - Los fondos en la tarjeta del cliente son autorizados y capturados por el Braintree, y un pedido y una factura se crean en el administrador de su tienda. **_Nota:_** Esto era `Authorize and Capture` en 2.3.x y versiones anteriores. |
| [!UICONTROL Button Color] | Sitio web | Determina el color de [!DNL Google Pay] botón. Opciones: `White` / `Black` |
| [!UICONTROL Merchant ID] | Vista de tienda | El ID proporcionado por Google debe introducirse aquí. |
| [!UICONTROL Accepted Cards] | Sitio web | Seleccione el tipo de tarjetas que un cliente puede utilizar para realizar un pedido mediante [!DNL Google Pay]. |
...
| Campo | [Ámbito](../../getting-started/websites-stores-views.md#scope-settings) | Descripción |
|--- |--- |--- |
| [!UICONTROL Enable Venmo through Braintree] | Sitio web | Determina si [!DNL Venmo] se incluye como forma de pago a través del Braintree. Opciones: `Yes` / `No` |
| [!UICONTROL Payment Action] | Sitio web | Determina la acción realizada por el Braintree cuando se procesa un pago. Opciones: <br/>**`Authorize`**- Los fondos en la tarjeta del cliente están autorizados, pero no transferidos desde la cuenta del cliente. Se crea un pedido en el administrador de la tienda. Más tarde puede capturar la venta y crear una factura.<br/>**`Intent Sale`** - Los fondos en la tarjeta del cliente son autorizados y capturados por el Braintree, y un pedido y una factura se crean en el administrador de su tienda. **_Nota:_** Esto era  _Autorizar y capturar_ en 2.3.x y versiones anteriores. |
...
| [!UICONTROL Payment Action] | Sitio web | Determina la acción realizada por PayPal a través del Braintree cuando se procesa un pago. Opciones: <br/>**`Authorize`**- Los fondos en la tarjeta del cliente están autorizados, pero no transferidos desde la cuenta del cliente. Se crea un pedido en el administrador de la tienda. Más tarde puede capturar la venta y crear una factura.<br/>**`Authorize and Capture`** - PayPal autoriza y captura los fondos de la tarjeta del cliente a través de Braintree, y se crea un pedido y una factura en el administrador de la tienda. |
...
| [!UICONTROL Display on Shopping Cart] | Sitio web | Determina si aparece el botón PayPal en la [minicarrito](../../stores-purchase/cart-configuration.md#mini-cart) y en el [carro de compras](../../stores-purchase/cart.md) página. Opciones: `Yes` / `No` |

{style="table-layout:auto"}

### [!UICONTROL Styling]

![Estilo de PayPal](./assets/payment-methods-braintree-paypal-styling.png)<!-- zoom -->
...
| Campo | [Ámbito](../../getting-started/websites-stores-views.md#scope-settings) | Descripción |
|--- |--- |--- |
| [!UICONTROL PayPal Button Type] | Sitio web | Establece el botón en uno de tres tipos: `PayPal Button` / `PayPal Pay Later Button` / `PayPal Credit Button` |
...
| [!UICONTROL Size] | Sitio web | Determina el tamaño del botón PayPal. Opciones: `Medium` / `Large` / `Responsive` |
...
| Campo | [Ámbito](../../getting-started/websites-stores-views.md#scope-settings) | Descripción |
|--- |--- |--- |
| [!UICONTROL Show PayLater Messaging] | Sitio web | Activa la mensajería PayAfter en la ubicación seleccionada. Opciones: `Yes` / `No`. Cuando está habilitado, muestra la mensajería PayAfter para las ofertas disponibles ([se aplican restricciones](https://developer.paypal.com/docs/checkout/pay-later/us/)). |
...
| Campo | [Ámbito](../../getting-started/websites-stores-views.md#scope-settings) | Descripción |
|--- |--- |--- |
| [!UICONTROL Name] | Vista de tienda | El descriptor Nombre consta de dos partes, separadas por un asterisco (*). La primera parte del descriptor identifica la compañía o DBA y la segunda parte identifica el producto. Por ejemplo: `company*myproduct`  <br/><br/>La longitud de las partes Company y Product del descriptor se puede asignar de las siguientes maneras, para una longitud combinada de hasta 22 caracteres: <br/>**`Option 1`**- La empresa debe tener tres caracteres / El producto puede tener hasta 18 caracteres<br/>**`Option 2`** - La empresa debe tener siete caracteres / El producto puede tener hasta 14 caracteres <br/>**`Option 3`**- La empresa debe tener 12 caracteres / El producto puede tener hasta nueve caracteres |
