...
| [!UICONTROL Generation Method] | Vista de tienda | Determina cómo se genera el mapa del sitio XML. `Standard` usa el proceso tradicional de generación sincrónica y procesa todos los datos en la memoria, mientras que `Batch` usa un modo por lotes asincrónico optimizado para la memoria para obtener una mayor flexibilidad y escalabilidad. Esta opción está disponible a partir de la versión 2.4.9 de. Opciones: `Standard` / `Batch` |
