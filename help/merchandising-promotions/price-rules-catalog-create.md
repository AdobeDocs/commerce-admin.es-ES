...
1. En el _Administrador_ barra lateral, vaya a **[!UICONTROL Marketing]** > _[!UICONTROL Promotions]_>**[!UICONTROL Catalog Price Rule]**.

1. En la esquina superior derecha, haga clic en **[!UICONTROL Add New Rule]**.

   El _[!UICONTROL Rule Information]_incluye secciones ampliables para **[!UICONTROL Conditions]**y **[!UICONTROL Actions]**.

   ![Regla de precios de catálogo: información](./assets/price-rule-catalog-new-ee.png){width="700" zoomable="yes"}

1. Complete la **[!UICONTROL Rule Name]** y **[!UICONTROL Description]** campos.

   Estos campos son solo para referencia interna.

1. Configure las variables **[!UICONTROL Status]** de la regla de precios según sea necesario.

   El estado predeterminado es `Inactive`.

   >[!NOTE]
   >
   >Una vez creada la regla, su estado se puede actualizar cambiando el estado a `Active` o `Inactive` según sea necesario.

1. Seleccione el **[!UICONTROL Websites]** donde la regla va a estar disponible.

1. Seleccione el **[!UICONTROL Customer Groups]** a los que se aplica esta regla.
...
1. ![Magento Open Source](../assets/open-source.svg) (Solo Magento Open Source) Introduzca la variable **[!UICONTROL From]** y **[!UICONTROL To]** fechas para determinar cuándo está en vigor la regla de precio.

   Puede introducir las fechas o utilizar el **[!UICONTROL Calendar]** (![Icono de calendario](../assets/icon-calendar.png)) para elegir las fechas. Si deja las fechas en blanco, la regla se activa cuando se guarda la regla de precio.

1. Introduzca un número para establecer el **[!UICONTROL Priority]** de esta regla en relación con otras reglas.

   >[!NOTE]
   >
   >El _[!UICONTROL Priority]_la configuración es importante cuando el mismo producto del catálogo cumple las condiciones establecidas para más de una regla de precio. La regla con la configuración de prioridad más alta (siendo 1 la más alta) se activa para el producto.
...
>[!NOTE]
>
>Para aplicar una `Category` condición de atributo de producto a cualquier [paquete](../catalog/product-create-bundle.md) o [agrupado](../catalog/product-create-grouped.md) producto, todos los productos secundarios deben asignarse a la misma categoría para que la regla se aplique correctamente. Si no es así, puede utilizar un [Regla de precio del carro](price-rules-cart-create.md) promoción en su lugar.
...
1. Haga clic en _Añadir_ (![Icono Agregar](../assets/icon-add-green-circle.png)) icono al principio de la línea siguiente y seleccione una opción para la condición, como un atributo de producto o una combinación.

1. En la lista debajo de **[!UICONTROL Product Attribute]**, elija el atributo que desea utilizar como base de la condición.
   ...
   >[!NOTE]
   >
   >Para que un atributo aparezca en la lista, debe configurarse para su uso en condiciones de regla promocional. Para obtener más información, consulte [Atributos del producto](../catalog/product-attributes.md).
...
1. Clic **[!UICONTROL is]** y elija el operador de comparación que describe la condición que se debe cumplir.
...
1. Para agregar otra línea de condición a la instrucción, haga clic en _Añadir_ (![Icono Agregar](../assets/icon-add-green-circle.png)) y elija una de las siguientes opciones:
   ...
   Si en cualquier momento desea eliminar parte de la instrucción condition, haga clic en **[!UICONTROL Delete]** (![Icono Eliminar](../assets/icon-delete-red-circle.png) al final de la línea.
...
1. Expandir ![Selector de expansión](../assets/icon-display-expand.png)el **[!UICONTROL Actions]** y haga lo siguiente:
   ...
   ![Regla de precio de catálogo: acciones](./assets/price-rule-catalog-actions.png){width="600" zoomable="yes"}

1. En **[!UICONTROL Pricing Structure Rules]**, configurado **[!UICONTROL Apply]** a uno de los siguientes:
   ...
   - `Adjust final price to discount value` - Establece el precio final en una cantidad fija y descontada. Por ejemplo: introduzca 20 en Importe de descuento para un precio final de 20,00 $.

   >[!NOTE]
   >
   >_Precio normal_ se refiere al precio base del producto sin ningún precio avanzado (especial/nivel/grupo) ni descuentos promocionales. _Precio final_ se refiere al precio con descuento que aparece en el carro de compras. <br/>El **_final_** el precio del producto se calcula como **_minimum_** precio pertinente, utilizando la fórmula siguiente: <br/>`Final Price=Min(Regular(Base) Price, Group(Tier) Price, Special Price, Catalog Price Rule) + Sum(Min Price per each required custom option)`
...
1. Introduzca el **[!UICONTROL Discount Amount]**.

1. Para detener el procesamiento de otras reglas después de aplicar esta regla, establezca **[!UICONTROL Discard Subsequent Rules]** hasta `Yes`.

   >[!NOTE]
   >
   >Estableciendo esto en `Yes` es una salvaguarda para evitar que el sistema aplique múltiples descuentos (reglas) al mismo producto.

## Paso 4: Agregar bloques dinámicos relacionados

{{ee-feature}}

[Bloques dinámicos](../content-design/dynamic-blocks.md) Las que estén asociadas a una regla de precio de catálogo aparecen en la tienda siempre que se cumplan las condiciones. Este es un paso opcional.

1. Expandir ![Selector de expansión](../assets/icon-display-expand.png)el **[!UICONTROL Related Dynamic Blocks]** sección.

1. Utilice el [filtros de búsqueda](../getting-started/admin-workspace.md) para localizar los bloques dinámicos que desea asociar a la regla.
...
1. En el _Cambios programados_ , haga clic en **[!UICONTROL Schedule New Update]** en la parte superior del cuadro).

   Si la regla tiene una actualización programada existente, puede hacer clic en **[!UICONTROL View/Edit]** a la derecha del cambio enumerado.

   Puede editar la actualización existente o asignar la regla de precio de catálogo a otra campaña. El **Editar actualización existente** está seleccionada de forma predeterminada.

1. Para programar la regla, introduzca el **[!UICONTROL Start Date]** y **[!UICONTROL End Date]** que la regla de precio debe estar activa.

   Puede introducir las fechas o elegir las fechas en el _Calendario_ (![Icono de calendario](../assets/icon-calendar.png)).
...
1. En el _Información de regla_ , establezca la sección **[!UICONTROL Status]** hasta `active`.
...
1. Actualizar las propiedades de una regla:
   ...
   - ![Adobe Commerce](../assets/adobe-logo.svg) (Solo Adobe Commerce) Haga clic en **[!UICONTROL Edit]** para mostrar el _[!UICONTROL Rule Information]_página.

   - ![Magento Open Source](../assets/open-source.svg) (Solo Magento Open Source) Haga clic en la regla de la lista para mostrar el _[!UICONTROL Rule Information]_página.
...
| [!UICONTROL Priority] | Un número que indica la prioridad de esta regla en relación con otras. La prioridad más alta es la número 1. |
