...
description: Aprenda a utilizar las reescrituras de direcciones URL de productos para redirigir los vínculos a la URL de otro producto en su tienda de Commerce.
...
Antes de empezar, asegúrese de comprender exactamente lo que debe lograr la redirección. Piense en términos de _destino_ / _petición original_ o _redirigir a_ / _redirigir desde_. Aunque es posible que los visitantes sigan navegando a la página anterior desde motores de búsqueda o vínculos obsoletos, la redirección hace que la tienda cambie al nuevo destino.

If [redirecciones automáticas](url-redirect-product-automatic.md) están activados para su tienda, no es necesario crear una reescritura cuando un producto [Clave de URL](../catalog/catalog-urls.md) se ha cambiado.
...
Si no está seguro, abra cada página de producto de la tienda y copie la ruta de la barra de direcciones del explorador. Al crear una redirección de producto, puede incluir o excluir el [ruta de categoría](../catalog/catalog-urls.md). Para este ejemplo, creamos un redireccionamiento de producto sin una ruta de categoría.
...
1. En la esquina superior derecha de la página Reescrituras de URL, haga clic en **Añadir reescritura de URL**.

1. Establecer **[!UICONTROL Create URL Rewrite]** hasta `For product`.
...
1. Debajo del árbol de categorías, haga clic en **[!UICONTROL Skip Category Selection]**.
   ...
   ![Reescritura de URL del producto: omitir selección de categorías](./assets/url-rewrite-skip-category-selection.png){width="600" zoomable="yes"}
   ...
   - Si tiene varias vistas de tienda, establezca **[!UICONTROL Store]** a la vista donde se aplica la reescritura. De lo contrario, se crea una reescritura para cada vista.

   - Para **[!UICONTROL Request Path]**, reemplace el valor predeterminado introduciendo la clave URL y el sufijo (si corresponde) de la solicitud de producto original. Este es el _redirigir desde_ producto que identificó en el paso de planificación.
   ...
   - Establecer **[!UICONTROL Redirect Type]** a uno de los siguientes:

      - `Temporary (302)`
      - `Permanent (301)`

   - Para su propia referencia, escriba un breve **[!UICONTROL Description]** de la reescritura.
...
1. Antes de guardar la redirección, revise lo siguiente:
   ...
   - La ruta de solicitud contiene la ruta del original _redirigir desde_ producto.
...
1. Realice una de las siguientes acciones:
   ...
   - Ir al original _redirigir desde_ página de solicitud de producto.
   - En la barra de direcciones del explorador, introduzca la ruta al original _redirigir desde_ inmediatamente después de la URL de la tienda y pulse **Entrar**.
...
| [!UICONTROL Request Path] | El producto que se va a redirigir. Según la configuración, la ruta de solicitud puede incluir lo siguiente `.html` o `.htm` sufijo y categoría. La ruta de solicitud debe ser única y no puede estar siendo utilizada por otra redirección. Si recibe un error que indica que la ruta de solicitud existe, elimine la redirección existente e inténtelo de nuevo. |
...
1. En _[!UICONTROL PRODUCTS INFORMATION]_, haga clic en **[!UICONTROL Websites]**pestaña.

1. En el _[!UICONTROL Add Product To Websites]_, seleccione todos los sitios web para los que desea restaurar las reescrituras de URL.
...
![Actualizar atributos: actualice varias reescrituras de URL](./assets/url-rewrites-update.png){width="600" zoomable="yes"}
