...
| `product_type` | Indica el tipo de producto. Valores:<br/>`simple` — Artículos materiales que se venden normalmente como unidades individuales o en cantidades fijas.<br/>`grouped` — Un grupo de productos separados que se vende como un conjunto.<br/>`configurable` — Un producto con múltiples opciones que el cliente debe seleccionar antes de realizar una compra. El inventario se puede administrar para cada conjunto de variaciones porque representan un producto independiente con un SKU distinto. Por ejemplo, una combinación de color y tamaño para un producto configurable está asociada a un SKU específico del catálogo.<br/>`virtual` — Un producto no tangible que no requiere envío y no se mantiene en inventario. Algunos ejemplos son servicios, suscripciones y suscripciones.<br/>`bundle` — Un conjunto de productos personalizables de productos simples que se venden juntos. |
| `categories` | Indica cada categoría asignada al producto. Separe las categorías y subcategorías con una barra diagonal. Para indicar varias rutas de categorías, separe cada ruta con una barra vertical símbolo |. Por ejemplo: `Default Category/Gear&#124;Default Category/Gear/Bags` |
...
| `description` | La descripción del producto proporciona información detallada sobre el producto y puede incluir etiquetas de HTML sencillas. |
...
| product_online | Determina si el producto está disponible para la venta en la tienda. Valores:<br/>`1` — (Sí) El producto está activado y disponible para la venta.<br/>`2` — (No) El producto está desactivado y no está disponible para la venta. |
| `tax_class_name` | El nombre de la clase de impuestos asociada a este producto. |
| `visibility` | Determina si el producto está visible en el catálogo y si está disponible para la búsqueda. Valores:<br/>`Not Visible Individually` — El producto no se incluye en las listas de productos, aunque puede estar disponible como variación de otro producto.<br/>`Catalog` — El producto aparece en todas las listas de catálogos.<br/>`Search` : el producto está disponible para operaciones de búsqueda.<br/>`Catalog, Search` — El producto está incluido en los listados del catálogo y también está disponible para la búsqueda. |
...
| save_rewrites_history | Cuando se proporciona con valor `1` con un nuevo `url_key`, se genera una nueva reescritura de URL 301 para que la URL antigua se redirija a la nueva. |
...
| `map_enabled` | Determina si el precio mínimo anunciado está habilitado en la configuración. Valores:<br/>`1` — (Sí) El MAP está activado.<br/>`0` (o en blanco) — (No) MAP no está activado. |
| `gift_message_available` | Determina si se puede incluir un mensaje de regalo con la compra del producto. Valores:<br/>`1` — (Sí) La opción de incluir un mensaje regalo se presenta al cliente.<br/>`0` (o en blanco) — (No) La opción de incluir un mensaje de regalo no se presenta al cliente. |
...
| `page_layout` | Determina el diseño de página de la página de producto. Valores:<br/>`No layout updates` — No se realiza ningún cambio en el diseño de la página.<br/>`1 column` — aplica un diseño de una columna a la página del producto.<br/>`2 columns with left bar` — aplica un diseño de dos columnas con una barra lateral izquierda a la página del producto.<br/>`2 columns with right bar` — Aplica un diseño de dos columnas con una barra lateral derecha a la página del producto.<br/>`3 columns` — aplica un diseño de tres columnas a la página del producto.<br/>`empty` — aplica un diseño en blanco a la página del producto. |
...
| `msrp_display_actual_price_type` | Determina si el cliente puede ver el precio real de un producto. Valores:<br/>`In Cart` — muestra el precio real del producto en el carro de compras.<br/>`Before Order Confirmation` — Muestra el precio real del producto al final del proceso de cierre de compra, justo antes de que se confirme el pedido.<br/>`On Gesture` — muestra el precio real del producto en una ventana emergente cuando el cliente hace clic en el _Haga clic para ver el precio_ o _¿Qué es esto?_ vínculo. |
...
| `use_config_min_qty` | Determina si se utiliza el valor predeterminado de la configuración y corresponde a la casilla de verificación Usar configuración de configuración. Valores:<br/>`1` — (Sí) Se utiliza la configuración por defecto para el valor de este atributo.<br/>`0` (o en blanco) — (No) La configuración predeterminada se puede anular para el valor de este atributo. |
...
| `notify_on_stock_below` | Especifica el nivel de stock que déclencheur un _sin existencias_ notificación. |
...
| `manage_stock` | Determina si se utiliza el control de inventario para administrar el producto. Valores:<br/>`1` — (Sí) Activa el control total del inventario para administrar los niveles de stock del producto.<br/>`0` (o en blanco) — (No) El sistema no realiza un seguimiento del número de artículos que están actualmente en existencias. |
...
| `crosssell_position` | Determina la posición (criterio de ordenación) de los SKU que se muestran como productos de venta cruzada en la `crosssell_skus` columna. |
| `upsell_skus` | Enumera el SKU de cada producto que se ha identificado como una venta ascendente. |
| `upsell_position` | Determina la posición (criterio de ordenación) de los SKU que se muestran como productos de ampliación de venta en `upsell_skus` columna. |
...
| `_associated_sku` | El SKU de un producto asociado a un producto agrupado. | Productos agrupados <br/>Productos agrupados |
...
Importación/Exportación de Precios Avanzada le permite actualizar rápidamente la información de precios para grupos de productos y precios de nivel. El proceso para [importar](data-import.md) y [exportar](data-export.md) los datos de precios avanzados son los mismos que cualquier otro tipo de entidad. El archivo CSV de ejemplo contiene los precios de nivel y de grupo de cada tipo de producto que admite precios avanzados. Cambiar los precios avanzados no afecta al resto del registro del producto.
...
| `tier_price_customer` | Identifica el [grupos de clientes](../customers/customer-groups.md) donde los precios de nivel están disponibles. Por ejemplo: `-  ALL GROUPS -  NOT LOGGED IN -  General -  Wholesale -  Retailer` |
...
| `tier_price` | El precio de nivel descontado del producto. Para [agrupar productos](../catalog/product-create-bundle.md), el precio del nivel se calcula como un porcentaje. |
| `group_price_website` | El [código de sitio web](../stores-purchase/stores.md#add-websites) de cada sitio web en el que haya grupos de precios disponibles. Si especifica varios sitios web, sepárelos con una coma y sin espacios. Por ejemplo: `-  website1 -  All Websites [USD]` |
...
| `group_price` | El precio de grupo descontado del producto. Para [agrupar productos](../catalog/product-create-bundle.md), el precio del grupo se calcula como un porcentaje. |
