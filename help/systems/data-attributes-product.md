...
|--- |--- |
| `sku` | (Obligatorio) La unidad de mantenimiento de existencias es un identificador alfanumérico único que se utiliza para realizar el seguimiento del inventario. Un SKU puede tener hasta 64 caracteres de longitud. Por ejemplo: `sku123`<br/>**_Note:_** Un SKU con más de 64 caracteres hace que la importación falle. |
...
| `attribute_set_code` | Asigna el producto a un conjunto de atributos o plantilla de producto específicos, según el tipo de producto. Por ejemplo: `default`<br><br>Una vez creado el producto, el conjunto de atributos no se puede cambiar mediante la funcionalidad de importación. Sin embargo, puede cambiar el conjunto de atributos desde Admin y volver a exportar el producto para actualizar el archivo CSV |
| `product_type` | Indica el tipo de producto. Valores:<br/>`simple` — Artículos tangibles que normalmente se venden como unidades individuales o en cantidades fijas.<br/>`grouped` — Un grupo de productos separados que se vende como un conjunto.<br/>`configurable` — Un producto con múltiples opciones que el cliente debe seleccionar antes de realizar una compra. El inventario se puede administrar para cada conjunto de variaciones porque representan un producto independiente con un SKU distinto. Por ejemplo, una combinación de color y tamaño para un producto configurable está asociada a un SKU específico del catálogo.<br/>`virtual` — Un producto no tangible que no requiere envío y no se mantiene en inventario. Algunos ejemplos son servicios, suscripciones y suscripciones.<br/>`bundle` — Un conjunto de productos personalizables de productos simples que se venden juntos. |
| `categories` | Indica cada categoría asignada al producto. Separe las categorías y subcategorías con una barra diagonal. Para indicar varias rutas de categorías, separe cada una con un símbolo de barra vertical \|. Por ejemplo: `Default Category/Gear\|Default Category/Gear/Bags` |
...
| product_online | Determina si el producto está disponible para la venta en la tienda. Valores:<br/>`1` — (Sí) El producto está habilitado y disponible para la venta.<br/>`2` — (No) El producto está desactivado y no está disponible para la venta. |
...
| `meta_keywords` | Las palabras clave de Meta solo son visibles para los motores de búsqueda y algunos motores de búsqueda las ignoran. Elija palabras clave de alto valor, separadas por una coma. Por ejemplo: `keyword1`, `keyword2`, `keyword3` |
| `meta_description` | Las descripciones de Meta proporcionan una breve descripción del producto para las listas de resultados de búsqueda. Lo ideal es que una metadescripción tenga entre 150 y 160 caracteres de longitud, aunque el campo acepta hasta 255 caracteres. |
...
| `map_enabled` | Determina si el precio mínimo anunciado está habilitado en la configuración. Valores:<br/>`1` — (Sí) La asignación está habilitada.<br/>`0` (o en blanco) — (No) MAP no está activado. |
| `gift_message_available` | Determina si se puede incluir un mensaje de regalo con la compra del producto. Valores:<br/>`1` — (Sí) La opción para incluir un mensaje de regalo se presenta al cliente.<br/>`0` (o en blanco) — (No) La opción de incluir un mensaje de regalo no se presenta al cliente. |
...
| `msrp_display_actual_price_type` | Determina si el cliente puede ver el precio real de un producto. Valores:<br/>`In Cart` — Muestra el precio real del producto en el carro de compras.<br/>`Before Order Confirmation` — Muestra el precio real del producto al final del proceso de cierre de compra, justo antes de que se confirme el pedido.<br/>`On Gesture` — Muestra el precio real del producto en una ventana emergente cuando el cliente hace clic en _Hacer clic para ver el precio_ o _¿Qué es esto?_ vínculo. |
...
| `manage_stock` | Determina si se utiliza el control de inventario para administrar el producto. Valores:<br/>`1` — (Sí) Activa el control de inventario completo para administrar los niveles de stock del producto.<br/>`0` (o en blanco) — (No) El sistema no realiza un seguimiento del número de artículos que están actualmente en existencias. |
...
| `bundle_values` | Describe cada valor asociado con una opción de paquete. Por ejemplo: `name=Bundle Option One,name_store_view_1=Group 1,type=dropdown; required=1,sku=sku-option2,price=10,default=0,default_qty=1.0000,price_type=fixed,can_change_qty=0,price_website_website_1=30.000000,price_type_website_website_1=fixed`<br/><br/>En la opción `name_[store-view-code]`, `[store-view-code]` es el nombre de la vista de la tienda.<br/></br>Las opciones `price_website_[website-code]` y `price_type_website_[website-code]` especifican el precio del producto para el sitio web `[website-code]`. Si el [ámbito del precio de catálogo](../catalog/catalog-price-scope.md) está establecido en *Global*, estas opciones no están disponibles. |
